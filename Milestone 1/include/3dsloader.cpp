...
char Load3DS (obj_type_ptr p_object, char *p_filename)
{
	...
	//while(!EOF)
	{
		...
		int ret = fread (&l_chunk_id, 2, 1, l_file); //Read the chunk header
		//printf("ChunkID: %x\n",l_chunk_id); 
...
//                printf("Number of vertices: %d\n",l_qty);
                for (i=0; i<l_qty; i++)
                {
					ret = fread (&p_object->vertex[i].x, sizeof(float), 1, l_file);
 					//printf("Vertices list x: %f\n",p_object->vertex[i].x);
                    ret = fread (&p_object->vertex[i].y, sizeof(float), 1, l_file);
 					//printf("Vertices list y: %f\n",p_object->vertex[i].y);
					ret = fread (&p_object->vertex[i].z, sizeof(float), 1, l_file);
...
//                printf("Number of polygons: %d\n",l_qty); 
                for (i=0; i<l_qty; i++)
                {
					ret = fread (&p_object->polygon[i].a, sizeof (unsigned short), 1, l_file);
					//printf("Polygon point a: %d\n",p_object->polygon[i].a);
					ret = fread (&p_object->polygon[i].b, sizeof (unsigned short), 1, l_file);
					//printf("Polygon point b: %d\n",p_object->polygon[i].b);
					ret = fread (&p_object->polygon[i].c, sizeof (unsigned short), 1, l_file);
			...
			case 0x4140:
				...
				for (i=0; i<l_qty; i++)
				{
					ret = fread (&p_object->mapcoord[i].u, sizeof (float), 1, l_file);
					//printf("Mapping list u: %f\n",p_object->mapcoord[i].u);
                    ret = fread (&p_object->mapcoord[i].v, sizeof (float), 1, l_file);
